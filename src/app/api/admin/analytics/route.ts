...
export const GET = handler(async () => {
  ...
  const responded = matches.filter((m) =>
    ([MatchStatus.ACCEPTED, MatchStatus.DECLINED, MatchStatus.PROPOSAL_REQUESTED, MatchStatus.PROPOSAL_SENT] as MatchStatus[]).includes(m.status)
  );
  ...
  return ok({
    ...
    dist,
    weekly,
    // Subscription revenue is out of MVP scope (README §14.6) — placeholders.
